...
def test_version(host, get_vars):
    ...
    version = local_facts(host=host, fact="logstream_exporter").get("version")

    install_dir = get_vars.get("logstream_exporter_install_path")
    defaults_dir = get_vars.get("logstream_exporter_defaults_directory")
    config_dir = get_vars.get("logstream_exporter_config_dir")
...
def test_open_port(host, get_vars):
    ...
    logstream_exporter_server = get_vars.get("logstream_exporter_server", {})

    print(logstream_exporter_server)

    if isinstance(logstream_exporter_server, dict):
        listen_address = logstream_exporter_server.get("metrics_listen_address")

    if not listen_address:
        listen_address = "127.0.0.1:9212"
